...
use super::manifest::{ProposalManifest, GovernanceOperation, ApprovalThresholds, VotingWindow, TimelockConfig, ExecutionCondition};
...
impl GovernanceEngine {
    ...
    pub fn submit_approval(&mut self, attestation: ApprovalAttestation) -> Result<()> {
        let id = &attestation.proposal_id;
        ...
        let signer = self.signers.get(&attestation.signer).context("Signer not registered")?;
        if !signer.is_active {
        ...
        if current_approvals.iter().any(|a| a.signer == attestation.signer) {
    ...
    pub fn update_status(&mut self, proposal_id: &str) -> Result<()> {
        ...
        if current_status == ProposalStatus::Pending {
            if now >= manifest.voting_window.start_time {
                self.statuses.insert(proposal_id.to_string(), ProposalStatus::Active);
            }
        }
        ...
        if current_status == ProposalStatus::Active || current_status == ProposalStatus::Pending {
            if now > manifest.voting_window.end_time {
                if total_weight >= manifest.thresholds.required_weight {
                    self.statuses.insert(proposal_id.to_string(), ProposalStatus::Succeeded);
                } else {
                    self.statuses.insert(proposal_id.to_string(), ProposalStatus::Defeated);
                }
    ...
    pub fn queue_proposal(&mut self, proposal_id: &str) -> Result<()> {
        ...
        self.statuses.insert(proposal_id.to_string(), ProposalStatus::Queued);
        self.save()?;
    ...
    pub fn execute_proposal(&mut self, proposal_id: &str) -> Result<()> {
        ...
        match status {
            ...
            ProposalStatus::Queued => {
                ...
                if now.signed_duration_since(queued_at).num_seconds() < timelock.delay_seconds as i64 {
                ...
                if let Some(max_delay) = timelock.max_delay_seconds {
                    if now.signed_duration_since(queued_at).num_seconds() > max_delay as i64 {
                        self.statuses.insert(proposal_id.to_string(), ProposalStatus::Expired);
                        self.save()?;
        ...
        self.statuses.insert(proposal_id.to_string(), ProposalStatus::Executed);
    ...
    pub fn cancel_proposal(&mut self, proposal_id: &str, reason: &str) -> Result<()> {
        ...
        self.statuses.insert(proposal_id.to_string(), ProposalStatus::Canceled);
        
        let mut manifest = self.get_manifest(proposal_id)?.clone();
        manifest.metadata.insert("cancellation_reason".to_string(), reason.to_string());
        manifest.metadata.insert("canceled_at".to_string(), Utc::now().to_rfc3339());
        self.proposals.insert(proposal_id.to_string(), manifest);
    ...
    pub fn audit(&self, proposal_id: Option<&str>) -> Result<String> {
        if let Some(id) = proposal_id {
            ...
            for a in approvals {
                audit_log.push_str(&format!(" - Signer: {}, Weight: {}, Time: {}\\n", a.signer, a.weight, a.timestamp));
            }
            ...
            let mut audit_log = format!("Governance System Audit\\n");
